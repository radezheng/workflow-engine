...
class Doctor:
    def __init__(self, *, repo: Path | None = None, config_path: Path | None = None, fix: bool = False) -> None:
        self.repo = repo.expanduser().resolve() if repo else discover_repo(Path.cwd())
        self.config_path = config_path.expanduser().resolve() if config_path else _default_config_path(self.repo)
        self.fix = fix
        self.findings: list[DoctorFinding] = []
        self.config: HWEConfig | None = None
    ...
    def run(self) -> int:
        ...
        self.print_report()
        return 1 if any(finding.level == "fail" for finding in self.findings) else 0

    def ok(self, check: str, message: str) -> None:
        self.findings.append(DoctorFinding("ok", check, message))

    def warn(self, check: str, message: str, fix: str | None = None) -> None:
        self.findings.append(DoctorFinding("warn", check, message, fix))

    def fail(self, check: str, message: str, fix: str | None = None) -> None:
        self.findings.append(DoctorFinding("fail", check, message, fix))
    ...
    def print_report(self) -> None:
        ...
        for finding in self.findings:
            print(f"[{labels[finding.level]}] {finding.check}: {finding.message}")
            if finding.fix:
                print(f"      fix: {finding.fix}")
        failures = sum(1 for finding in self.findings if finding.level == "fail")
        warnings = sum(1 for finding in self.findings if finding.level == "warn")
        ok = len(self.findings) - failures - warnings
        print(f"\nSummary: {failures} failure(s), {warnings} warning(s), {ok} ok.")
    ...
    def _check_repo(self) -> None:
        if self.repo is None:
            self.fail("repo", "Could not find an HWE repository from the current directory.", "Run from the HWE repo root, pass --repo, or set HWE_REPO.")
            return
    ...
    def _check_hwe_command(self) -> None:
        ...
        if os.environ.get("HWE"):
            candidates.append(Path(os.environ["HWE"]).expanduser())
        if self.repo is not None:
            candidates.append(self.repo / ".venv" / "bin" / "hwe")
        path_hwe = shutil.which("hwe")
        if path_hwe:
            candidates.append(Path(path_hwe))
        for candidate in candidates:
            if candidate.exists() and os.access(candidate, os.X_OK):
                self.ok("hwe-command", f"HWE executable: {candidate}")
                return
        self.warn("hwe-command", "No executable HWE command found among expected candidates.", "Activate the HWE virtualenv, reinstall with `pip install -e .`, or set HWE=/path/to/hwe.")
    ...
    def _check_paths(self) -> None:
        ...
        if template_root is None:
            self.warn("prompt_template_root", "No prompt template root configured.")
    ...
    def _check_database(self) -> None:
        ...
        try:
            ...
            return
        if database.get("password") or database.get("password_env") or database.get("password_command"):
    ...
    def _check_postgres_login(self, database: dict[str, Any]) -> None:
        ...
        if not password and database.get("password_env"):
            password = os.environ.get(str(database["password_env"]))
    ...
    def _check_profiles(self) -> None:
        ...
        for name, profile in sorted(profiles.items()):
            ...
            command_name = shlex.split(str(command))[0] if str(command).strip() else ""
            if command_name and (Path(command_name).expanduser().exists() or shutil.which(command_name)):
            ...
            for switch_command in profile.get("switch_commands") or []:
                raw_command = str(switch_command.get("command", "")) if isinstance(switch_command, dict) else str(switch_command)
    ...
    def _check_ai_providers(self) -> None:
        assert self.config is not None
        for name, provider in sorted((self.config.ai_providers or {}).items()):
            if not isinstance(provider, dict):
                self.fail(f"ai_provider:{name}", "AI provider config is not a mapping.")
                continue
            if provider.get("api_key_env") and not os.environ.get(str(provider["api_key_env"])):
                self.warn(f"ai_provider:{name}:secret", f"api_key_env is set but not exported: {provider['api_key_env']}")
            if provider.get("base_url"):
                self._check_http_url(f"ai_provider:{name}:base_url", str(provider["base_url"]), required=False)
...
def _default_config_path(repo: Path | None) -> Path | None:
    if os.environ.get("HWE_CONFIG"):
        return Path(os.environ["HWE_CONFIG"]).expanduser()
    if repo is not None:
        return repo / "hwe.config.yaml"
    return None
