...
def plan_workitem(project_ref: str, workitem_id: str, request: PlanWorkitemRequest) -> dict[str, Any]:
    ...
    if planner_task is None:
        planner_task = storage.create_task(
            ...
            prompt_text="Create an execution strategy and propose the next task graph for this workitem.",
            skills=["hwe"],
            outputs=["workflow plan", "task graph proposal"],
            gates=["plan addresses requirements", "tasks have roles and dependencies", "verification tasks are included"],
            priority=10,
...
def materialize_plan(project_ref: str, task_id: str, request: MaterializePlanRequest) -> dict[str, Any]:
    ...
    created = storage.create_task(
        ...
        outputs=["task graph created in HWE"],
        gates=["plan file reviewed", "tasks created with correct roles and dependencies", "verification tasks included"],
        skills=["hwe"],
...
def _plan_breakdown_prompt(project_root: Path, project_id: str, workitem: dict[str, Any], plan_task: dict[str, Any], stdout_path: Path) -> str:
    return "\n".join(
        [
            ...
            f"Workitem title: {workitem['title']}",
            f"Workflow id: {plan_task['workflow_id']}",
            f"Source planning task id: {plan_task['id']}",
            f"Plan stdout path: {stdout_path}",
            "",
