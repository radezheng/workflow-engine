...
2. **Plan And Design**: For a new or unclear request, start with `designer/workitem-plan`. For an existing codebase or any change with architectural risk, follow with `designer/technical-design`; existing projects should research files, commands, tests, patterns, and constraints before implementation tasks are chosen. Use `designer` only if HWE will actually route that profile and its required skills are verified; otherwise use `default` and record the same outputs. Planning/design tasks must not be treated as the executable queue.
3. **Task Queue**: Materialize a concrete queue through a dedicated `designer/task-breakdown` task that receives the successful plan/design run's `stdout.log` path and creates HWE task records with `hwe task create`. The designer must verify available profiles and choose real routable profiles, verify each task's prompt template, and create project-local template overrides under `.engine/prompt-templates/<role>/<name>.md` when a public template is not suitable. Do not have the default coordinator manually parse/transcribe planner prose into tasks except under explicit owner direction for recovery. Prefer several narrow `impl`/`fix` tasks, deterministic `command` checks, `http_check` smoke tests for runnable apps, and final review/acceptance tasks. Every implementation task should name intended files, constraints, dependencies, and verification.
...
- Initial planning for new, unclear, or product-shaped requests: use `designer/workitem-plan`; it distinguishes new project vs existing project and proposes research/design/task candidates without creating the executable queue.
- Technical design for existing projects, architecture changes, or non-trivial implementation: use `designer/technical-design`; it should research existing code, commands, tests, patterns, and constraints before proposing task candidates. If research is too large, create a separate `researcher/source-research` or designer research task first.
- Task graph materialization: use `designer/task-breakdown` after a planning/design task succeeds, pass the plan/design `stdout.log` path, and require the worker to create HWE task records through CLI/API operations. The designer must verify profiles and prompt templates before creating tasks, and may create project-local prompt template overrides under `.engine/prompt-templates/<role>/<name>.md` when needed.
