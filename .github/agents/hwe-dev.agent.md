...
Public repository: `https://github.com/radezheng/workflow-engine`. For Hermes/default-profile bootstrapping, install or update from that repo, reinstall with `pip install -e .`, sync `.agents/skills/hwe/` into the active Hermes skill directory, then run doctor before mutating workflow state. Detailed steps live in `docs/hermes-bootstrap.md`.
